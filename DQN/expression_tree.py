...
def init_multitree(n_trees : int):
    '''
    ...
    for _ in range(n_trees):
        multitree.children.append(init_tree())
...
def get_multitree_preorder_travs(multitree):
    '''
    ...
    for i in range(multitree.n_trees):
        preorder_trav = []
        get_tree_preorder_travs(multitree.children[i], preorder_trav)
        multitree_preorder_travs.insert(preorder_trav, i)

def get_tree_preorder_travs(tree_root_node, preorder_trav=[]):
    '''
    ...
    for child in tree_root_node.children:
        if child.arity > 0:
            preorder_trav.append(get_tree_preorder_travs(child, preorder_trav))
        else:
...
def update_tree(tree_root_node, action):
    ...
    if (tree_root_node.arity > 0) and (len(tree_root_node.children) == 0):
        ...
        if len(tree_root_node.children) == 0:
            ...
            # if the node has a child (which can only be a left child) then do the same for that child. 
